...
from infragraph.blueprints.devices.dell.xe9685L import Dellxe9685L
...
async def test_dellxe9680L(count, nic_device):
    """From a Dellxe9685L device, generate a graph and validate the graph.
    ...
    print(f"\ndevice {device.name} nic={nic_device} count={count}")
